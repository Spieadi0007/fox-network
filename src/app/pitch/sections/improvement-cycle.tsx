        ...
        <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewportConfig}
            ...
            {nodes.map((n, i) => (
              <motion.div key={i} variants={fadeInUp} className="flex items-center gap-4">
                <div className="flex flex-col items-center">
                  <div className="flex h-16 w-16 items-center justify-center rounded-xl border border-stone-700 bg-stone-900/60">
                    <n.icon className="h-7 w-7 text-fox-orange" strokeWidth={1.5} />
                  </div>
                  <p className="mt-2 text-center text-[11px] leading-tight text-stone-400 whitespace-pre-line">{n.label}</p>
                </div>
                {i < nodes.length - 1 && (
                  <div className="hidden text-stone-600 sm:block">
          ...
          <motion.div variants={fadeInUp} className="mt-8 flex justify-center">
            <div className="flex items-center gap-3 rounded-full border border-fox-orange/20 bg-fox-orange/5 px-5 py-2">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#e8590c" strokeWidth="1.5" strokeLinecap="round">
